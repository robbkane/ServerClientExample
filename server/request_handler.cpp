...
void request_handler::handle_request(const request& req, reply& rep)
{
  ...
  { // BT call block...
	  ...
	  // Grab the correlation header info, note that this bit isn't included in the BT timing.
      for (const auto& header : req.headers) 
      {
          if (header.name == "singularityheader") 
    	  {
    		correlation_header = header.value;
    	  }
      }
      ...
      appd::sdk::BT bt(request_path, correlation_header);
    
      bt.set_url(request_path);
      bt.add_user_data("server_user_data", "server_mcserverface");
    
      std::cout << std::endl << "Got request " << req_cnt++ << ": " << request_path << std::endl;   //RKane
        
      // ExitCall call block...
      {
         appd::sdk::ExitCall ec(bt, "CES_Example_Java_Backend_Name");
         ...
         const std::string& appd_correlation_header = ec.get_correlation_header();
          ...
          // Fill out the reply to be sent to the client. Or not, if we are passing the request on to Java  //RKANE
          rep.status = reply::ok;
          char buf[512];
      ...
      } // end of ExitCall call block.

	  bt.override_time_ms(5);
